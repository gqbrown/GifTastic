...
function putButtonsOnScreen () {
	...
	sports.forEach(function(sports, index) {
		var button = $("<button>");
		button.attr("class", "btn btn-primary sports-buttons");
		button.attr("id", sports);
		button.text(sports);
...
var sports = ["Football", "Baseball", "Basketball", "Hockey", "Tennis"];
putButtonsOnScreen();

// wait for submit.  When submit is clicked create a new button for 

$('#sportsFormSubmit').on("click", function(e) {

	e.preventDefault();
	var newsports = $('#sportsInput').val();
	sports.push(newsports);
     console.log("Here is the new sports list");
  	putButtonsOnScreen();
	$('#sportsInput').val("");
});
...
$(document).on("click",".sports-buttons", function(e) {

	console.log(e.target.id);
	var sports = e.target.id;
